...
export const createDefaultConnector = (key = DEFAULT_CONNECTOR_KEY, overrides?: Partial<Record<string, unknown>>) => {
...
export const ensureGoogleSheetsShape = (settingsDoc: any) => {
  ...
  const normalizeConnectorArray = (
    ...
    const connectors = Array.isArray(connectorsRaw) ? (connectorsRaw as Array<Record<string, unknown>>) : [];
    ...
    for (const connector of connectors) {
      ...
      const key = String(next.key ?? DEFAULT_CONNECTOR_KEY).trim() || DEFAULT_CONNECTOR_KEY;
      const definition = getConnectorDefinition(key);
      ...
      const label = String(next.label ?? definition.label).trim() || definition.label;
      const mapping = mapToPlain(next.mapping);
      const transformations = normalizeTransformations(next.transformations);
  ...
  gs.oauth.sources = (gs.oauth.sources as Array<Record<string, unknown>>).map((source) => {
    const nextSource = source as Record<string, unknown>;
    if (!nextSource._id) {
      const legacyId = String(nextSource.sourceId ?? nextSource.id ?? '').trim();
      nextSource._id = Types.ObjectId.isValid(legacyId) ? new Types.ObjectId(legacyId) : new Types.ObjectId();
      touched = true;
    }

    nextSource.connectors = normalizeConnectorArray(nextSource.connectors, () => {
      const legacySpreadsheetId = String(nextSource.spreadsheetId ?? '').trim();
      const legacyRange = String(nextSource.range ?? '').trim();
      const legacySheetName =
        legacyRange.split('!')[0]?.replace(/^'/, '').replace(/'$/, '').trim() ||
      ...
      return createDefaultConnector(DEFAULT_CONNECTOR_KEY, {
        ...
        transformations: normalizeTransformations(nextSource.transformations)
      });
    });
  ...
  gs.shared.profiles = (gs.shared.profiles as Array<Record<string, unknown>>).map((profile) => {
    const nextProfile = profile as Record<string, unknown>;
    if (!nextProfile._id) {
      const legacyId = String(nextProfile.profileId ?? nextProfile.id ?? '').trim();
      nextProfile._id = Types.ObjectId.isValid(legacyId) ? new Types.ObjectId(legacyId) : new Types.ObjectId();
      touched = true;
    }

    nextProfile.connectors = normalizeConnectorArray(nextProfile.connectors, () => {
      const legacySpreadsheetId = String(nextProfile.spreadsheetId ?? '').trim();
      const legacySheetName = String(nextProfile.sheetName ?? '').trim() || 'Sheet1';
      if (!legacySpreadsheetId) return null;
      const lastMapping = nextProfile.lastMapping as Record<string, unknown> | undefined;
      return createDefaultConnector(DEFAULT_CONNECTOR_KEY, {
        label: getConnectorDefinition(DEFAULT_CONNECTOR_KEY).label,
        ...
        mapping: mapToPlain(nextProfile.columnsMap ?? lastMapping?.columnsMap),
        transformations: normalizeTransformations(lastMapping?.transformations),
  ...
  const oauthActiveId = String(gs.oauth.activeSourceId?.toString?.() ?? gs.oauth.activeSourceId ?? '').trim();
  if (oauthActiveId) {
    const exists = (gs.oauth.sources as Array<Record<string, unknown>>).some(
      (source) => String((source as Record<string, unknown>)._id ?? '').trim() === oauthActiveId,
    );
    if (!exists) {
  ...
  if (!gs.oauth.activeSourceId && gs.oauth.sources.length > 0) {
    gs.oauth.activeSourceId = (gs.oauth.sources[0] as Record<string, unknown>)._id;
    touched = true;
  }

  const sharedActiveId = String(gs.shared.activeProfileId?.toString?.() ?? gs.shared.activeProfileId ?? '').trim();
  if (sharedActiveId) {
    const exists = (gs.shared.profiles as Array<Record<string, unknown>>).some(
      (profile) => String((profile as Record<string, unknown>)._id ?? '').trim() === sharedActiveId,
    );
    if (!exists) {
  ...
  if (!gs.shared.activeProfileId && gs.shared.profiles.length > 0) {
    gs.shared.activeProfileId = (gs.shared.profiles[0] as Record<string, unknown>)._id;
    touched = true;
  }
  ...
  const hasReadyConnector = (
    ...
    const activeIdStr = String(activeId?.toString?.() ?? activeId ?? '').trim();
    if (!activeIdStr) return false;
    const container = containers.find((entry) => String((entry as Record<string, unknown>)._id ?? '').trim() === activeIdStr);
    if (!container) return false;
    const key = String(activeConnectorKey ?? DEFAULT_CONNECTOR_KEY).trim() || DEFAULT_CONNECTOR_KEY;
    ...
    return Boolean(connector.enabled ?? true) && Boolean(spreadsheetId) && Boolean(sheetName) && Object.keys(mapping).length > 0;
  };

  const oauthReady = hasReadyConnector(gs.oauth.sources, gs.oauth.activeSourceId, gs.oauth.activeConnectorKey);
  const sharedReady = hasReadyConnector(gs.shared.profiles, gs.shared.activeProfileId, gs.shared.activeConnectorKey);
...
export const getOrCreateSettings = async (companyId: string, userId: string) => {
  ...
  if (typeof settings.isModified === 'function' && settings.isModified('googleSheets')) {
    await settings.save();
  }
  return settings;
};
...
export const getConnectorFromContainer = (
  ...
  const connectors = Array.isArray(container.connectors) ? container.connectors : [];
  const normalizedKey = String(connectorKey || DEFAULT_CONNECTOR_KEY).trim() || DEFAULT_CONNECTOR_KEY;
  const existing = connectors.find((connector) => String(connector.key ?? '').trim() === normalizedKey);
  ...
  const stored = (container.connectors ?? []).find(
    (connector) => String((connector as Record<string, unknown>).key ?? '').trim() === normalizedKey
  );
...
export const normalizeTransformations = (value: unknown): Record<string, unknown> => {
  if (!value) return {};
  if (value instanceof Map) {
...
export const idEquals = (left: unknown, right: string) => {
  if (!left) return false;
  const leftId = left instanceof Types.ObjectId ? left.toString() : String(left);
  return leftId === String(right).trim();
};

export const asObjectId = (value?: string | null) => {
  if (!value) return null;
  return new Types.ObjectId(String(value).trim());
};
