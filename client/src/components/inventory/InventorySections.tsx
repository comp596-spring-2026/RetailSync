...
import { itemsApi, locationsApi } from '../../api';
...
export const InventorySections = () => {
  ...
  const loadByLocation = async () => {
    ...
    try {
      const res = await locationsApi.byLocation(locationCode.trim());
  ...
  return (
    <>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Stack direction="row" spacing={2} flexWrap="wrap">
          ...
          <TextField
            ...
            type="number"
            value={move.qty}
            onChange={(e) => setMove((prev) => ({ ...prev, qty: e.target.value }))}
          />
          <TextField
            label="Notes"
            value={move.notes}
            onChange={(e) => setMove((prev) => ({ ...prev, notes: e.target.value }))}
          />
      ...
      <LoadingEmptyStateWrapper
        ...
        empty={!loadingStock && (!stock || stock.items.length === 0)}
        loadingLabel="Loading location stock..."
        emptyMessage={!locationCode.trim() ? 'Enter a location code and click Load' : 'No stock at this location'}
        emptySecondary={!locationCode.trim() ? '' : 'Try another location or add items.'}
      >
        <Paper sx={{ p: 2 }}>
