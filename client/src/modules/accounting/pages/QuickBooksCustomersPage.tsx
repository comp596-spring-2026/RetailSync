...
export const QuickBooksCustomersPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await accountingApi.getQuickbooksHubEntities('customer', {
        page,
        pageSize,
      ...
      setError(extractApiErrorMessage(apiError, 'Failed to load QuickBooks customers'));
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Stack spacing={2}>
      <PageHeader
        ...
        subtitle="Search, filter, and review customer references from the connected QuickBooks company."
        icon={<PeopleAltIcon />}
      />
      <QuickBooksTabs />
      ...
      >
        <Typography variant="body2" color="text.secondary">
          Customer rows are loaded from the QuickBooks hub entities endpoint.
        </Typography>
        <SmartTable
          rows={rows}
          columns={columns}
